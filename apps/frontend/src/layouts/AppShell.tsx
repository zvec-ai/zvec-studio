...
function DemoTabs(): JSX.Element {
  ...
  return (
    <div style={style}>
      ...
      >
        <button className="zv-detail-tab zv-detail-tab--active">{t('pages.collections.detail.tabs.overview')}</button>
...
export function AppShell(): JSX.Element {
  ...
  function handleCloseCollection(e: React.MouseEvent, name: string, path?: string) {
    e.stopPropagation();
    e.preventDefault();
    closeCollection.mutate({ name, path }, {
      onSuccess: () => {
  ...
  return (
    <div className="zv-shell" data-testid="app-shell">
      ...
      <aside className={`zv-sidebar${!pageTitle ? ' zv-sidebar--hidden' : sidebarCollapsed ? ' zv-sidebar--hidden' : ''}`}>
        ...
        <div className="zv-sidebar__section" data-tour="collections">
          <div className="zv-sidebar__section-head">
            <span className="zv-sidebar__section-label" onClick={() => navigate('/collections')} style={{ cursor: 'pointer' }}>{t('nav.collections')}</span>
        ...
        <div className="zv-sidebar__footer">
          <div className="zv-sidebar__footer-version">
            Zvec v{healthz.data?.zvecVersion ?? '…'}
          </div>
      ...
      <CreateEmbeddingDialog open={showCreateEmbedding} onClose={() => setShowCreateEmbedding(false)} />
      <CreateRerankerDialog open={showCreateReranker} onClose={() => setShowCreateReranker(false)} />
      ...
      <Dialog
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setDeleteEmbeddingTarget(null)} disabled={deleteEmbedding.isPending}>
              {t('actions.cancel')}
            </Button>
            <Button variant="danger" onClick={confirmDeleteEmbedding} loading={deleteEmbedding.isPending}>
      ...
      <Dialog
        ...
        footer={
          <>
            <Button variant="ghost" onClick={() => setDeleteRerankerTarget(null)} disabled={deleteReranker.isPending}>
              {t('actions.cancel')}
            </Button>
            <Button variant="danger" onClick={confirmDeleteReranker} loading={deleteReranker.isPending}>
